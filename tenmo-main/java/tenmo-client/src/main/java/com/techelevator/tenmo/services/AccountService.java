...
public class AccountService {
    ...
    public NewTransfer requestMoney(String token, int fromUserId, int toUserId, BigDecimal amount) {
        NewTransfer newTransfer = new NewTransfer();
        newTransfer.setFromUserId(fromUserId);
        newTransfer.setToUserId(toUserId);
        newTransfer.setAmount(amount);
        ...
        headers.setBearerAuth(token);
        HttpEntity entity = new HttpEntity(newTransfer, headers);
        ResponseEntity<NewTransfer> response = restTemplate.exchange(BASE_URL + "request-money", HttpMethod.POST, entity, NewTransfer.class);
        return response.getBody();
    }

    public NewTransfer[] getPendingRequests(String token) {
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(token);
        HttpEntity entity = new HttpEntity(headers);
        ResponseEntity<NewTransfer[]> response = restTemplate.exchange(BASE_URL + "view-requests", HttpMethod.GET, entity, NewTransfer[].class);
        return response.getBody();
    }



    public String approveTransfer(String token, int transferId) {
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(token);
        HttpEntity entity = new HttpEntity(headers);
        ResponseEntity<String> response = restTemplate.exchange(BASE_URL + "transfer/approve/" + transferId, HttpMethod.PUT, entity, String.class);
        return response.getBody();
    }

    public String rejectTransfer(String token, int transferId) {
        HttpHeaders headers = new HttpHeaders();
        headers.setBearerAuth(token);
        HttpEntity entity = new HttpEntity(headers);
        ResponseEntity<String> response = restTemplate.exchange(BASE_URL + "transfer/reject/" + transferId, HttpMethod.PUT, entity, String.class);
        return response.getBody();
    }
